...
import { deleteMasterAmenitiesFacility, getMasterAmenitiesFacilities, getMasterAmenitiesSubCategory, insertUpdateMasterAmenitiesFacilities } from "../../models";
...
export default function Index({ PageAccess }: any) {
  ...
  const [_formData, _setFormData] = useState<any>({ id: 0, subCategoryId: '', facilityName: '', isActive: true, notes: '' });

  const validate = {
    subCategoryId: { error: false, message: '' },
    facilityName: { error: false, message: '' },
  }
  ...
  const handleDeleteItem = (item: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteMasterAmenitiesFacility(item?.id)
          .then((resp) => {
  ...
  const handleGoBack = () => {
    ...
    _setFormData({ id: 0, subCategoryId: '', facilityName: '', isActive: true, notes: '' })
  }

  const checkValidation = () => {
    const validation = Object.assign(validate, {})
    let valid = true
    if (!_formData?.subCategoryId) {
      validation.subCategoryId.error = true;
      validation.subCategoryId.message = 'Required Field';
      valid = false
    }
    if (!_formData?.facilityName?.trim()) {
      validation.facilityName.error = true;
      validation.facilityName.message = 'Required Field';
      valid = false
    }
  ...
  const handleSubmitForm = () => {
    ...
    const body = {
      id: _formData?.id || 0,
      subCategoryId: _formData?.subCategoryId,
      facilityName: _formData?.facilityName,
      isActive: _formData?.isActive,
      notes: _formData?.notes || ""
    }

    insertUpdateMasterAmenitiesFacilities(body)
      .then((resp) => {
        if (resp?.data?.status === "success") {
          getGridList()
          handleGoBack()
  ...
  const getPrintTableHeadBody = () => {
    const header = ["S. No", "Sub-Category", "Facilities", "Status"];
  ...
  const exportEXCEL = () => {
    ...
    getExportEXCEL({ header, body, fileName: "Amenities Facilities" })
  }
          ...
          <div className="row justify-content-between align-items-center py-3">
            <div className="col-md-4 my-2">
              <div className="d-flex align-items-center gap-2 mobJustify">
                ...
                <span className="text-muted">Amenities Facilities</span>
              </div>
            </div>
            ...
            <Table sx={{ ...customTableTemplate }}>
              <TableHead>
                <TableRow sx={{ ...customTableHeader }}>
                  ...
                  <TableCell className="fw-bold">Facilities</TableCell>
                ...
                {_tableItems?.length > 0 ? (
                  _tableItems?.filter((content: any) => {
                    ...
                    <TableRow key={index} className="mt-2" style={{ border: "none", borderBottom: "1px solid #D2D2D2", }}>
                      ...
                      <TableCell className="text-muted bolder text-nowrap">{item?.facilityName}</TableCell>
                ...
                <div className="bg-field-gray d-flex flex-column justify-content-between">
                  <div className="row pb-3">
                    <div className="col-md-3 my-3">
                      ...
                      <FormControl fullWidth error={_validate?.subCategoryId?.error}>
                        <Select size="small" value={_formData?.subCategoryId} onChange={(e: any) => changeFormData('subCategoryId', e.target.value)}
                          style={{ backgroundColor: "#F3F3F3", }}>
                          <MenuItem value="Select" disabled>Select</MenuItem>
                          {_subCategoryList?.map((mItem: any) =>
                            mItem?.isActive && <MenuItem key={mItem?.id} value={mItem?.id}>{mItem?.subCategory}</MenuItem>
                          )}
                        </Select>
                        <FormHelperText className="text-danger">{_validate?.subCategoryId?.message}</FormHelperText>
                      </FormControl>
                    </div>
                    <div className="col-md-6 my-3">
                      <div className="text-muted fs14 mb-1">Facility</div>
                      <TextField fullWidth sx={{ ...textFieldStyle }}
                        value={_formData?.facilityName} onChange={(e: any) => changeFormData('facilityName', e.target.value)}
                        error={_validate?.facilityName?.error} helperText={_validate?.facilityName?.message} />
                    </div>
                  </div>
                  <div className="row">
                    <hr />
