...
import { deleteMasterIssueCategory, getMasterIssueCategory, insertUpdateMasterIssueCategory } from "../../models";
...
export default function Index({ PageAccess }: any) {
  ...
  const [_formData, _setFormData] = useState<any>({ id: 0, issueType: "", isActive: true, notes: '' });

  const validate = { issueType: { error: false, message: "" } };
  ...
  const handleDeleteItem = (item: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteMasterIssueCategory(item?.id)
          .then((resp) => {
  ...
  const handleGoBack = () => {
    ...
    _setFormData({ id: 0, issueType: "", isActive: true, notes: '' });
  };
  ...
  const checkValidation = () => {
    ...
    if (!_formData?.issueType?.trim()) {
      validation.issueType.error = true;
      validation.issueType.message = "Required Field";
      valid = false;
    }
  ...
  const handleSubmitForm = () => {
    ...
    insertUpdateMasterIssueCategory(body)
      .then((resp) => {
        if (resp?.data?.status === "success") {
          getGridList();
          handleGoBack();
  ...
  const getPrintTableHeadBody = () => {
    const header = ["S. No", "Issue Category", "Status"];
  ...
  const exportEXCEL = () => {
    ...
    getExportEXCEL({ header, body, fileName: "Issue Category" })
  };
        ...
        <>
          <div className="row justify-content-between align-items-center py-3">
            <div className="col-md-4 my-2 d-flex align-items-center gap-2">
              ...
              <span className="text-muted">Issue Categories</span>
            </div>
          ...
          <div className="bg-field-gray d-flex flex-column justify-content-between">
            <div className="row pb-3">
              <div className="col-md-6 mt-3">
                ...
                <TextField className="" fullWidth sx={{ ...textFieldStyle }} value={_formData?.issueType}
                  onChange={(e: any) => changeFormData("issueType", e.target.value)}
                  error={_validate?.issueType?.error} helperText={_validate?.issueType?.message} />
              </div>
            </div>
            <div className="row">
              <hr />
