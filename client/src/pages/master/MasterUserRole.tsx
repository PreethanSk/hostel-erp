...
import { deleteMasterUserRole, getMasterUserRole, insertUpdateMasterUserRole } from "../../models";
...
export default function Index({ PageAccess }: any) {
  ...
  const [_formData, _setFormData] = useState<any>({ id: 0, roleName: "", isActive: true, notes: "" });

  const validate = { roleName: { error: false, message: "" } };
  ...
  const handleGoBack = () => {
    ...
    _setFormData({ id: 0, roleName: "", isActive: true, notes: '' });
  };
  ...
  const checkValidation = () => {
    ...
    if (!_formData?.roleName?.trim()) {
      validation.roleName.error = true;
      validation.roleName.message = "Required Field";
      valid = false;
    }
  ...
  const handleSubmitForm = () => {
    ...
    insertUpdateMasterUserRole(body)
      .then((resp) => {
        if (resp?.data?.status === "success") {
          getGridList();
          handleGoBack();
  ...
  const getPrintTableHeadBody = () => {
    const header = ["S. No", "User Role", "Status"];
          ...
          <div className="bg-field-gray d-flex flex-column justify-content-between">
            <div className="row">
              <div className="col-md-6 my-3">
                <div className="text-muted fs14 mb-1">Role Name</div>
                <TextField fullWidth sx={{ ...textFieldStyle }}
                  value={_formData?.roleName} onChange={(e: any) => changeFormData('roleName', e.target.value)}
                  error={_validate?.roleName?.error} helperText={_validate?.roleName?.message} />
              </div>
            </div>
          </div>
          <div className="row">
            ...
            <div className="col-md-4 mb-2">
              <div className="d-flex align-items-center justify-content-end mobJustify gap-2">
                ...
                <Button variant="contained" color="primary" disabled={_loading} className="" onClick={handleSubmitForm}>{_formData?.id > 0 ? 'Update' : 'Save'}</Button>
