...
import { deleteMasterPageList, getMasterPageList, insertUpdateMasterPageList } from "../../models";
...
export default function Index({ PageAccess }: any) {
  ...
  const [_formData, _setFormData] = useState<any>({ id: 0, pageName: "", pageURL: "", isActive: true, notes: "" });

  const validate = {
    pageName: { error: false, message: "" },
  };
  ...
  const handleGoBack = () => {
    ...
    _setFormData({ id: 0, pageName: "", pageURL: "", isActive: true, notes: "" });
  };
  ...
  const checkValidation = () => {
    ...
    if (!_formData?.pageName?.trim()) {
      validation.pageName.error = true;
      validation.pageName.message = "Required Field";
      valid = false;
    }
  ...
  const handleSubmitForm = () => {
    ...
    const body = {
      id: _formData?.id || 0,
      pageName: _formData?.pageName,
      pageURL: _formData?.pageURL,
      isActive: _formData?.isActive,
      notes: _formData?.notes || ""
    };

    insertUpdateMasterPageList(body)
      .then((resp) => {
        if (resp?.data?.status === "success") {
          getGridList();
          handleGoBack();
  ...
  const getPrintTableHeadBody = () => {
    const header = ["S. No", "Page Name", "Page URL", "Status"];
  ...
  const exportEXCEL = () => {
    ...
    getExportEXCEL({ header, body, fileName: "Page List" })
  };
        ...
        <>
          <div className="row justify-content-between align-items-center py-3">
            <div className="col-md-4 my-2 d-flex align-items-center gap-2">
              ...
              <span className="text-muted">Page List</span>
            </div>
          ...
          <div className="bg-field-gray d-flex flex-column justify-content-between">
            <div className="row pb-3">
              <div className="col-md-4 my-3">
                <div className="text-muted fs14 mb-1">Page Name</div>
                <TextField fullWidth sx={{ ...textFieldStyle }}
                  value={_formData?.pageName} onChange={(e: any) => changeFormData('pageName', e.target.value)}
                  error={_validate?.pageName?.error} helperText={_validate?.pageName?.message} />
              </div>
              <div className="col-md-4 my-3">
                <div className="text-muted fs14 mb-1">Page URL</div>
                <TextField fullWidth sx={{ ...textFieldStyle }}
                  value={_formData?.pageURL} onChange={(e: any) => changeFormData('pageURL', e.target.value)} />
              </div>
            </div>
          </div>
          <div className="row">
