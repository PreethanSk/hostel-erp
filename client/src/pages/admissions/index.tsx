...
import { Button, Table, TableBody, TableCell, TableHead, TableRow, TextField, MenuItem, Pagination, PaginationItem, Tabs, Tab, FormControl, Select, TableContainer, Tooltip } from '@mui/material';
...
import { deleteCandidateAdmission, getAdmissionGridList, getBranchGridList, getBranchRoomsList, insertUpdateCandidateAdmission } from '../../models';
...
export default function Index({ PageAccess }: any) {
  ...
  const [_filterData, _setFilterData] = useState({ branchId: '', fromDate: moment().clone().startOf('month').format('YYYY-MM-DD'), toDate: moment().format('YYYY-MM-DD') });
  ...
  const [_formData, _setFormData] = useState<any>({ id: 0, edit: false, isActive: true, admissionType: 'Candidate' })
  ...
  const handleClearForm = () => {
    ...
    _setLoading(false);
    _setEditForm(false);
    _setFormData({ id: 0, edit: false, isActive: true, admissionType: 'Candidate' });
  }
  ...
  const getPrintTableHeadBody = () => {
    const header = ["S. No", "Status", "Branch", "Room No", "Room Type", "Cot No", "Cot Type", "Candidate Name", "Mobile Number", "Date of Admission", "Updated At"];
    const body = _tableItems?.map((item: any, index: number) => [
      index + 1,
      item?.admissionStatus || "Inprogress",
      item?.branchName,
      item?.roomNumber,
      item?.roomTypeName,
      item?.cotNumber,
  ...
  const handleSubmitForm = () => {
    ...
    const body = {
      ...
      admissionType: _formData?.admissionType || 'Candidate',
  ...
  const getGridList = () => {
    ...
    getAdmissionGridList(_page, _rowsPerPage, _filterData?.branchId?.toString(), _filterData?.fromDate, _filterData?.toDate)
      .then((resp) => {
        if (resp?.data?.status === "success") {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteCandidateAdmission(item?.id)
          .then((resp) => {
            if (resp?.data?.status === "success") {
              CustomAlert("success", "Admission details removed");
            }
          })
          .catch((err) => {
            console.log(err)
              ...
              _tableItems?.filter((content: any) => {
                ...
                <TableRow key={index} sx={{ ...CustomTableHover }}>
                  ...
                  <TableCell className="text-muted" align="center">
                    {PageAccess === 'Write' && <div className="d-flex align-items-center gap-2">
                      <Tooltip title='edit'>
                        <img role="button" onClick={() => handleUpdateItem(item)} src={IMAGES_ICON.EditIcon} draggable='false' />
                      </Tooltip>
                      {item?.admissionStatus === 'Rejected' && <Tooltip title='edit'>
                        <img height="18" role="button" onClick={() => handleDeleteItem(item)} src={IMAGES_ICON.DeleteIcon} draggable='false' />
                      </Tooltip>
                      }
                    </div>}
                  </TableCell>
                  <TableCell className="text-muted text-nowrap">
                    <div className={`fs14 text-uppercase text-center px-2 pt-1 ${item?.admissionStatus === "Submitted" ? 'statusSubmitted' :
                      item?.admissionStatus === "Rejected" ? 'statusRejected' :
                        item?.admissionStatus === "Approved" ? 'statusResolved' :
                          'statusInProgress'}`}>{item?.admissionStatus || "Inprogress"}</div>
                  </TableCell>
        ...
        <div className="bg-field-gray  border rounded px-4 py-1">
          ...
          <div className="row justify-content-between pt-2">
            <div className="col-md-2 my-2">
              ...
              {_formData?.edit ? <span className="ms-4">{_formData?.branchName}</span> : <FormControl className="ms-4 w-75" variant="standard" fullWidth>
            ...
            <div className="col-md-2 my-2">
              ...
              {_formData?.edit ? <span className="ms-4">{_formData?.roomNumber}</span> : <FormControl className="ms-4 w-75" variant="standard" fullWidth>
            ...
            <div className="col-md-2 my-2">
              ...
              {_formData?.edit ? <span className="ms-4">{_formData?.cotNumber}</span> : <FormControl className="ms-4 w-75" variant="standard" fullWidth>
            ...
            <div className="col-md-2 my-2">
              ...
              <TextField className="ms-4 w-75" fullWidth sx={{ ...textFieldStyle, padding: "8px 0px" }} variant="standard" type="date" onKeyDown={DisableKeyUpDown}
                ...
                slotProps={{ input: { readOnly: _formData?.edit } }} />
            </div>
            {/* <div className="col-md-2 my-2">
              <div className="text-muted fs14 mb-1 d-flex align-items-center gap-1">Admission Type</div>
              {_formData?.edit ? <span className="ms-4 text-capitalize">{admissionDetails?.admissionDetails?.admissionType || _formData?.admissionType || 'Candidate'}</span> :
                <FormControl className="ms-4 w-75" variant="standard" fullWidth>
                  <Select value={_formData?.admissionType || 'Candidate'}
                    label=" " sx={{ fontSize: "14px" }} disableUnderline
                    onChange={(e: any) => changeFormData('admissionType', e.target.value)}>
                    <MenuItem className="fs14" value={'Candidate'}>Candidate</MenuItem>
                    <MenuItem className="fs14" value={'Staff'}>Staff</MenuItem>
                  </Select>
                </FormControl>}
            </div> */}
            {_formData?.id ? <div className="col-md-2 my-2">
              ...
              <div className="fs14 mb-1 ms-4 w-75">{_formData?.admittedBy || _formData?.candidateName}</div>
            </div> :
          ...
          <div className="my-3">
            <div className={_formData?.id ? '' : 'd-none'}>
              ...
              <div className="p-4">
                ...
                {_selectedTab === 7 && <Payments handleClose={handleGoBack} PageAccess={PageAccess} handleBack={() => _setSelectedTab(6)} handleNext={handleGoBack} />}
              </div>
            </div>
          </div>
        </div>
